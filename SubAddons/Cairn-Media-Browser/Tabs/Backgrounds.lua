--[[
Cairn-Media-Browser / Tabs / Backgrounds

Browses every background texture reachable to the running client
(Cairn-Media private + public + third-party LSM). Each row shows
visibility badge + display name + file path + a 220x60 sample panel
filled with the background. Hover for the exact code snippet.

Cairn-Media-Browser/Tabs/Backgrounds (c) 2026 ChronicTinkerer. MIT license.
]]
...
local function buildList()
	local out = {}
	for _, name in ipairs(Media:ListBackgrounds("private")) do
		out[#out + 1] = { displayName = name, path = Media:GetBackground(name),
			source = "private", cairnName = name }
	end
	for _, name in ipairs(Media:ListBackgrounds("public")) do
		out[#out + 1] = { displayName = name, path = Media:GetBackground(name),
			source = "public", cairnName = name, lsmName = "Cairn " .. name }
	end
	if LSM then
		for _, lsmName in ipairs(LSM:List("background") or {}) do
			if not lsmName:match("^Cairn ") then
				out[#out + 1] = { displayName = lsmName, path = LSM:Fetch("background", lsmName),
...
local function tooltipCodeFor(entry)
	if entry.source == "lsm" then
		return {
			...
			'frame:SetBackdrop({',
			('  bgFile = LSM:Fetch("background", %q),'):format(entry.lsmName),
			'})',
		}
	end
	return {
		...
		'frame:SetBackdrop({',
		('  bgFile = Media:GetBackground(%q),'):format(entry.cairnName),
		'})',
	}
end
...
local function buildRow(parent, entry)
	...
	row._sample:SetBackdropColor(1, 1, 1, 1)
	...
	if not row._tooltipHooked then
		...
		row:HookScript("OnEnter", function(self)
			...
			GameTooltip:AddLine(("background: %s"):format(e.displayName), 1, 1, 1)
...
local function build(pane, browser)
	local _, body = browser:BuildTabShell(pane, "Backgrounds")
	...
	browser:AppendIntro(body,
		("Showing %d backgrounds (Cairn-private, Cairn-public, third-party LSM). "
...
Browser:RegisterTab("backgrounds", {
	label = "Backgrounds",
	order = 40,
	build = build,
})
