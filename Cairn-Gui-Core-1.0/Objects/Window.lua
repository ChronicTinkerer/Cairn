...
local methods = {
	...
	['ApplySettings'] = function(self)
		...
		-- Midnight (Dragonflight 10.0+): SetMinResize / SetMaxResize were
		-- merged into SetResizeBounds(minW, minH, maxW, maxH).
...
local function Constructor()
	...
	frame:SetScript("OnSizeChanged", function(this,width,height)		
		self.settings.width 	= Tools:Round(width)
		self.settings.height	= Tools:Round(height)
	...
	titleBar:SetScript("OnMouseUp", function(this)
		...
		self.settings.top 		= Tools:Round(frame:GetTop())
		self.settings.left	 	= Tools:Round(frame:GetLeft())
