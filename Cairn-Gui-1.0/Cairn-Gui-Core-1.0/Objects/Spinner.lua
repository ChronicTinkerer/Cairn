...
local methods = {		
	...
	['ApplySettings'] = function(self)		
		...
		local buttonHeight	= Tools:Round(settings.height / 2)
		-- Update Settings
	...
	['OnSpin'] = function(self,delta)
		...
		if delta > 0 then
			newNumber = self:ProcessValue(Tools:Round(oldNumber,settings.step)+settings.step)												
		else
			newNumber = self:ProcessValue(Tools:Round(oldNumber,settings.step)-settings.step)								
		end
	...
	['UpdateBar'] = function(self)
		...
		local width = Tools:Round(((number - settings.min) / (settings.max - settings.min)* settings.barWidth ))	
		if width == 0 then 
			self.bar:Hide()
		else
			self.bar:Show()
