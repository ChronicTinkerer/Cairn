...
- **Cairn-Media-1.0 MINOR=3** — added Material Symbols icon font family + icon glyph registry. Three font styles ship as PUBLIC entries (Outlined / Rounded / Sharp) — variable fonts only (~11-15 MB each), the static-instance variants from the Google Fonts download were stripped (~200 MB removed). Bundled under Apache 2.0 with `LICENSE.txt` shipping alongside each font per license terms. New icon-glyph registry: `lib._icons` maps short names to Unicode codepoints in the PUA range (U+E000 - U+F8FF). 53 commonly-needed UI / WoW icons pre-registered (navigation: `close` / `menu` / `arrow_back` / `chevron_*` / `expand_*` / `first_page` / `last_page` / etc.; actions: `add` / `remove` / `check` / `cancel` / `delete` / `edit` / `save` / `refresh` / `search` / `settings` / `filter_list` / `sort` / `done`; status: `info` / `warning` / `error` / `help` / `check_circle`; toggles: `visibility` / `lock` / `star` / `favorite` / `bookmark`; objects: `folder` / `description` / `home` / `person` / `group` / `notifications`; WoW-flavored: `emoji_events` / `bolt` / `shield` / `speed` / `map` / `explore` / `timer` / `schedule`). New API: `:GetIconCodepoint(name)`, `:GetIconGlyph(name)` returns the UTF-8 string ready for `FontString:SetText` (uses `utf8.char` on modern clients with a manual 1/2/3-byte fallback for older flavors), `:HasIcon`, `:ListIcons`, `:IterIcons`, `:RegisterIcon(name, codepoint)` for consumer extensions. Render pattern: `fs:SetFont(Media:GetFont("MaterialOutlined"), 16, ""); fs:SetText(Media:GetIconGlyph("close"))`.
- **Cairn-Media-1.0 MINOR=2** — new module: dual-mode media registry (private + public). Curated set of fonts (`Default` / `Numeric` / `Heading` / `Combat`), statusbar textures (`Plain` / `Solid`), borders (`Tooltip` / `Dialog`), backgrounds (`Tooltip` / `Dialog` / `Solid`), and sounds (`Alert` / `Notify` / `LevelUp`) accessible by short name via `Cairn.Media:GetFont(name)` etc. **Two visibility modes:** PRIVATE entries are stored only in the lib's internal table — visible to `Cairn.Media` consumers only, NOT registered with LibSharedMedia-3.0. PUBLIC entries are stored internally AND registered with LSM under `"Cairn <ShortName>"` so WeakAuras / ElvUI / Details! and any LSM consumer sees them in their media dropdowns. Lookup is uniform (`:GetFont(name)` checks both buckets, public wins on collision); listings can filter by visibility (`:ListFonts("public")` / `:ListFonts("private")` / `:ListPublicFonts()` / `:ListPrivateFonts()`). Initial release ships WoW built-in paths as PRIVATE entries (no binary files shipped); the public bucket starts empty. Drop-in folders for free-license additions: `CairnMedia/Assets/Private/<type>/` and `CairnMedia/Assets/Public/<type>/` for each of the five media types, all with `.gitkeep` files so the structure persists in git. Public API: `:GetFont` / `:GetStatusbar` / `:GetBorder` / `:GetBackground` / `:GetSound`, `:ListFonts(visibility?)` / etc., `:ListPublicFonts` / `:ListPrivateFonts` filtered helpers per type, `:Has(type, name)`, `:IsPublic(type, name)`, `:Iter(type, visibility?)` for sorted `(name, path)` walking. Hot-reload safe via shared `_media` table.
- **Cairn-Gui-Theme-Default-2.0 MINOR=2** — soft-dep on `Cairn-Media-1.0`. `font.body` / `font.heading` / `font.small` tokens now resolve to Cairn-Media's `Default` and `Heading` entries when the lib is loaded (the standalone configuration), with clean fallbacks to `STANDARD_TEXT_FONT` and the canonical `Fonts\MORPHEUS.TTF` path when embedded consumers omit `CairnMedia/`. No token / call-site changes for theme consumers.
- **Cairn-Deque-1.0 MINOR=1** — new module: double-ended queue with O(1) push/pop at both ends.

### Fixed

- **Cairn-Gui-Widgets-Standard-2.0 MINOR=6** — Window `OnDragStop` now NORMALIZES the post-drag anchor back to CENTER-relative coords before firing `"Moved"`. WoW's `StartMoving` / `StopMovingOrSizing` typically rewrites the frame's anchor to a `BOTTOMLEFT`-relative spec (often `BOTTOMLEFT, UIParent, BOTTOMLEFT, x, y`). MINOR=5 fired `"Moved"` with those raw `GetPoint(1)` coords, so consumers that persisted `(x, y)` and restored via `SetPoint("CENTER", UIParent, "CENTER", x, y)` saw the window jump on `/reload` (anchor types mismatched). MINOR=6 computes the frame's center vs `UIParent` center (scale-corrected), `ClearAllPoints`, re-anchors to `CENTER`, then fires `"Moved (dx, dy, 'CENTER', 'UIParent', 'CENTER')"`. The contract for consumers becomes: "(x, y) is the offset from `UIParent` center". Vellum's `db.profile.panel.x/y` round-trips without changes; persistence now actually works across `/reload`.

- **Cairn-Gui-Widgets-Standard-2.0 MINOR=5 + Cairn-Gui-Theme-Default-2.0 MINOR=3** — three Vellum-build framework gaps shipped (memory: `cairn_gui_2_vellum_framework_gaps`).
  - **Window self-anchors on Acquire** to `SetPoint("CENTER", UIParent, "CENTER")` when the consumer hasn't set an anchor. Was: invisible (rendered at `(0,0)` of UIParent which is the bottom-left corner offscreen). Vellum's per-build `SetPoint` workaround is no longer needed for visibility, only for applying the saved drag offset.
  - **Window fires `"Moved"` event after OnDragStop** with args `(x, y, point, relTo, relPoint)`. Lets consumers persist drag position via `widget.Cairn:On("Moved", function(_, x, y) db.profile.<thing>.x, .y = x, y end)`. Was: drag worked visually but no callback, so no way to round-trip position to saved variables without monkey-patching the title bar.
  - **`texture.icon.check` token swapped** from atlas `"common-icon-checkmark"` (silently invisible on Interface 120005) to file path `Interface\Buttons\UI-CheckBox-Check` (Blizzard-shipped, stable since Classic). Resolves the "Checkbox toggled state shows no glyph" bug.

- **Cairn-Gui-2.0 Core MINOR=20** — three small lib improvements identified during the Cairn-Media-Browser build (2026-05-08).
  - **`Mixins/Base.Release` fires a `"Release"` event** before nuking the subscription registry (Step 2.4, between detach-from-parent and clear-subscriptions). Lets multiple consumers register cleanup via `widget.Cairn:On("Release", fn)` without clobbering the existing single-method `OnRelease()` override. Cleanup runs before pool return.
  - **Extended docstring on `Release`** explaining the raw-children-don't-cascade gotcha. WoW frames can't destroy regions, so naive consumer code that calls `:CreateFontString` / `:CreateTexture` / `BackdropTemplate` `CreateFrame` on every Acquire accumulates stale anonymous regions on pooled frames (visible symptom: text overlay / ghosting after pool recycle). Documents the canonical stash-and-reuse pattern and points at `Button.lua`'s `_label` as the in-tree example.
  - **`Cairn-Demo` `Console:Clear` bug fix** — was calling `line.Release` (frame, nil) and silently no-op'ing instead of releasing pooled lines. Now calls `line.Cairn:Release()`. Same bug pattern that hit Cairn-Media-Browser earlier in this round; both fixes ship together.

- **Cairn-Gui-Widgets-Standard-2.0 MINOR=4** — `Button.OnAcquire` now calls `frame:RegisterForClicks("AnyUp")`. The Primitives layer's `OnMouseDown` / `OnMouseUp` HookScripts swallow `OnClick` on Interface 120005 unless the Button is registered for the click type explicitly. Without this, `btn.Cairn:On("Click", fn)` silently never fires — consumers had to add per-button `:RegisterForClicks` workarounds (Vellum's `fixButtonClicks(row)` helper; Cairn-Media-Browser's visibility filter buttons that prompted this fix). Resolves the long-tracked Vellum-build framework gap. `"AnyUp"` covers left / right / middle so consumers can dispatch on the `button` arg passed to the Click handler.

### Added (internal)

- **Cairn-Media-Browser** — new internal SubAddon under `Cairn/SubAddons/Cairn-Media-Browser/`. Live in-game viewer over everything `Cairn-Media-1.0` has registered. Slash `/cmb` (alias `/cairn-media`). Seven tabs: Welcome (per-type counts + theme tie-in note), Fonts (live preview at 12 / 16 / 24 pt; icon-glyph sample for Material* fonts), Statusbars (3-width tinted swatches), Borders (220×60 sample frame via `BackdropTemplate`), Backgrounds (220×60 untinted panel fill), Sounds (Play button per row, `PlaySoundFile` on the Master channel), Icons (font picker over Outlined / Rounded / Sharp + 5-column glyph grid with name + Unicode codepoint underneath each). Visibility filter row (All / Public / Private) above each media-type list; re-renders rows in place when toggled. Built on `Cairn-Gui-2.0` widgets plus raw WoW Texture / FontString children for previews. MIT. Not in `.pkgmeta` or `release.ps1` (matches the `Cairn-Demo` / `Cairn-Gui-Demo-2.0` internal-only decision). Single instance covers deque, queue (FIFO), and stack (LIFO) usage via method aliases (`Enqueue`/`Dequeue`/`Peek` for queue; `Push`/`Pop`/`Top` for stack; `PushBack`/`PushFront`/`PopBack`/`PopFront`/`PeekBack`/`PeekFront` for deque). Optional `capacity` with `onFull = "drop"` (evicts opposite end and fires `Dropped`) or `"error"` (raises). Lazy subscription handlers (`OnPushed` / `OnPopped` / `OnDropped` / `OnEmptied`) — zero per-instance memory until anyone subscribes; each `:OnX` returns an unsubscribe closure, plus `:UnsubscribeAll(owner)` for mass cleanup. Backed by the classic two-index scheme (`_first` / `_last`); indexes auto-reset to `0/-1` on natural drain or `Clear()` so they don't drift. Hot-reload safe via the shared `_Deque` prototype trick. No hard load-order constraints. Use cases: ring-buffer log tails, work queues with back-pressure, undo/redo stacks, BFS/DFS frontiers, recent-items caches.
