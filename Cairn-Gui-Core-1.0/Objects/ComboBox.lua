...
local methods = {
	...
	['SetList'] = function(self,list,orderedKeys)
		...
		for position, key in ipairs(orderedKeys) do
			local comboBoxItem = Gui:Create('ComboBoxItem')
...
local function Constructor()
	...
	editBox:SetScript('OnEnterPressed',  function(this)
		if this:GetText() == self.settings.oldValue then return end
		self:FireEvent("OnRenameValue",self.settings.selectedKey,this:GetText())
		Gui:ClearFocus()	
	end)
